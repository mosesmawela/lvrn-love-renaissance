  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
          ...
          >
            ...
            <div className="p-3 border-b border-white/10 flex justify-between items-center bg-white/5">
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full animate-pulse ${isAiAvailable ? 'bg-green-500' : 'bg-red-500'}`} />
            ...
            <div className="p-3 border-t border-white/10 bg-black/40">
              <div className="relative">
                <input
                  ...
                  placeholder={isAiAvailable ? "Enter command..." : "System Offline"}
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className={`pointer-events-auto w-14 h-14 rounded-full backdrop-blur-md border flex items-center justify-center text-white shadow-lg group ${!isAiAvailable ? 'bg-red-900/50 border-red-500/50' : 'bg-black/50 border-white/20'
          }`}
        aria-label="Toggle AI Assistant"
      >
        <AnimatePresence mode="wait">
          {isOpen ? <X size={24} /> : (
            !isAiAvailable ? <AlertTriangle size={24} className="text-red-400" /> : <Zap size={24} className="group-hover:text-yellow-400 transition-colors" />
          )}
        </AnimatePresence>
      </motion.button>
    </div>
