...
export const ParticleBackground: React.FC<ParticleBackgroundProps> = ({ intensity = 0.5, className = "" }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particles.forEach(p => {
        p.x += p.speedX + (Math.random() - 0.5) * (intensity * 0.5); // React to intensity
        p.y += p.speedY + (Math.random() - 0.5) * (intensity * 0.5);
